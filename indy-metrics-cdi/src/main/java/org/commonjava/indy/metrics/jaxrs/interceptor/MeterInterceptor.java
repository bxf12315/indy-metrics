...
public class MeterInterceptor
{
    ...
    @AroundInvoke
    public Object operation( InvocationContext context )
            throws Exception
    {
        ...
        List<Timer.Context> timers = Stream.of( measures.timers() ).map(named->util.getTimer( metrics, measures, named ).time()).collect(
                Collectors.toList());
